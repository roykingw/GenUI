...
//angular.module('inspinia').controller('dashboardViewCtrl', function ($timeout, $rootScope, $scope, $state, $stateParams, $http, ModalUtils, chartService, $interval, $uibModal, dataService) {
angular.module('inspinia').controller('dashboardViewCtrl', function ($timeout, $rootScope, $scope, $state, $stateParams, $http, chartService, $interval) {
    ...
    $http.get("dashboard/getDatasetList.do").success(function (response) {
    	...
    	console.info("加载cboard报表报错");
    	$scope.errorInfo="加载cboard报错，请确认CBoard运行情况";
    	return;
    });
...
//            ModalUtils.alert("Export error, please ask admin to check server side log.", "modal-warning", "lg");
            swallerror("Export error, please ask admin to check server side log.", "modal-warning", "lg");
        });
    };
    ...
    $http.get("dashboard/getBoardParam.do?boardId=" + $stateParams.id).success(function (response) {
        ...
        if (response) {
            $scope.boardParams = response.config;
        } else {
            $scope.boardParams = [];
        }
    });
